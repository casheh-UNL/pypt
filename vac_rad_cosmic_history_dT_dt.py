...
# from .eff_DoF.eff_DoF import g_SM as gstar_sm
...
# radiation domination near critical temperature
def time_to_temp(t, gstar=GSTAR_SM):
    return sqrt(sqrt(90/pi**2/gstar) * M_pl / t)
def temp_to_time(T):
    return sqrt(90/pi**2 / gstar_sm(T)) * M_pl / T**2
def H_rad(T):
    return sqrt(pi**2 / 90 / M_pl**2 * gstar_sm(T) * T**4)


# def default_dT_dt_from_self(func):
#     '''Decorator to replace a default "None" value with self.previous_dT_dt'''
#     @wraps(func)
#     def wrapper(self, tau, y, dT_dt=None, *args, **kwargs):
#         if dT_dt is None:
#             dT_dt = self.previous_dT_dt
#         return func(self, tau, y, dT_dt, *args, **kwargs)
#     return wrapper
...
class CosmicHistoryVacuumRadiation_dT_dt:
    def __init__(self, Veff, vw):
        ...
        elif type(Veff) == VeffBL:
            self.dV0 = Veff.find_minimum_phi(T=0)
            self.sigma = Veff.find_surface_tension()
            self.Tc = Veff.find_critical_phi_and_temperature()[1]
            self.VEV = Veff.find_minimum_phi
        ...
        self.previous_dT_dt = sqrt(self.c) * self.temperature_to_tau(1) * (-H_rad(self.Tc)*self.Tc) # start with radiation domination
    ...
    def dV(self, phi, T):
        ...
        elif type(self.Veff) == VeffBL:
            return -self.Veff(phi, T)
    ...
    def rate(self, T):
        if type(self.Veff) == VEffGeneric:
            ...
            S3_T = abs(self.bounce_action_cosmoTransitions(T))
